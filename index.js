  ...
  check_session(request.cookies)
    .then(
      (cur_sess) => {
        ...
        db.query(
          "SELECT b.*, (SELECT COUNT(*) FROM users u) total_users, (SELECT users.avatar FROM users where users.id = :cur_user) cur_avatar FROM (SELECT (@rownum:=@rownum + 1) total_rank, ugu.* FROM ( SELECT user_id, avatar, username, IFNULL(SUM(job_likes), 0) total_likes, COUNT(jobs_id) total_jobs FROM (SELECT users.avatar avatar, users.username username, users.id user_id, jobs.id jobs_id, jobs.likes job_likes FROM users LEFT JOIN jobs ON users.id = jobs.user_id) a, (SELECT @rownum:=0) r GROUP BY user_id ORDER BY total_likes DESC ) ugu) b WHERE b.user_id = :profile_user",
...
app.post("/upload_job", (request, response) => {
  check_session(request.cookies)
    .then(
      (cur_sess) => {
        db.query(
          "SELECT count(*) cnt FROM jobs WHERE jobs.status in (:statuses) and jobs.user_id = :user_id",
          { statuses: ADDEBLAE_STATUSES, user_id: cur_sess.user_id },
          (err, row) => {
            if (err) return response.status(500).send(RES_FAIL);
            if (row.cnt >= 2) return response.status(200).json({ reason: "Max submits count reached (MAX 2)" });

            const FILE_LIST = Object.values(request.files);
            if (FILE_LIST > 5) return response.status(500).json({ reason: "To many files!" });
...
function clear_expired_sessions() {
  db.query(
    ...
    { time: Math.floor(Date.now() / 1000) - SESSION_LENGTH },
    (err) => {
      if (err) console.log("Error during expired session clearing: ", err);
    }
  );
}
